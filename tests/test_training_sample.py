...
def test_optional_fields_are_absent_until_populated():
    sample = TrainingSample(state="position")

    assert sample.state == "position"
    assert not hasattr(sample, "reference_value")
...
def test_to_moves_tensor_fields_without_touching_python_values():
